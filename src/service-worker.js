...
self.addEventListener('message', (event) => {
  ...
  if (event.data && event.data.type === 'SKIP_WAITING') {
    ...
    if (event.ports && event.ports[0]) {
      ...
      console.error('[Service Worker] No message port available to respond');
    }
...
self.addEventListener('push', (event) => {
  ...
  const title = data.title || 'LyticSpend Reminder';
  const options = {
    body: data.body || 'Time to record your expenses!',
    icon: '/logo192.png',
    badge: '/logo192.png',
    data: data.data || {}
  };
